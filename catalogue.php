...
// Cerca PDF nella tabella ProdottoSingolo
$sql = "SELECT pdf FROM ProdottoSingolo WHERE pdf IS NOT NULL AND pdf != ''";
...
// Cerca PDF nella tabella Kit
$sql = "SELECT pdf FROM Kit WHERE pdf IS NOT NULL AND pdf != ''";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $pdfArray[] = $row['pdf'];
...
// Cerca PDF nella tabella ConfigurableProduct (ho assunto il nome della tabella)
$sql = "SELECT pdf FROM prodottoconfigurabile WHERE pdf IS NOT NULL AND pdf != ''";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $pdfArray[] = $row['pdf'];
...
foreach ($pdfArray as $pdfFile) {
    // Rimuove l'estensione .pdf e sostituisce _ con spazio
                ...
                <?php foreach ($pdfFiles as $pdfItem): ?>
                    <div class="col-span-1 flex flex-col items-center">
                        <div class="flex flex-col gap-[10px] items-center product cursor-pointer"
                            ...
                            <!-- Icona PDF invece dell'immagine -->
